...
def entropy(y):  
    ...
    return -np.sum(value * np.log(value + EPS))
    ...
    def choose_best_split(self, X_subset, y_subset):
        ...
        for i in range(X_subset.shape[1]):
          for j in np.unique(X_subset[:, i]):
            ...
            score = (y_left.shape[0] / X_subset.shape[0] * func(y_left) + 
            y_right.shape[0] / X_subset.shape[0] * func(y_right))
            if score < score_min:
              feature_index = i
              threshold = j
              score_min = score
