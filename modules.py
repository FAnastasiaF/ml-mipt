...
Original file is located at
    https://colab.research.google.com/drive/1TzBnZJQ7FF_K3E2Czd5GP0AeUY0ztuwC
...
class SoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        # Your code goes here. ################################################
        self.output = np.divide(np.exp(self.output), 
                                np.sum(np.exp(self.output), 
                                       axis=1, 
                                       keepdims=True))
        return self.output
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput -= self.output * np.sum(gradOutput * self.output, 
                                               axis=1, 
                                               keepdims=True)
        return self.gradInput
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        # Your code goes here. ################################################
        self.output = self.output - np.log(np.sum(np.exp(self.output), 
                                                  axis=1, 
                                                  keepdims=True))
        return self.output
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = gradOutput - np.exp(self.output) * np.sum(gradOutput, 
                                                                   axis=1, 
                                                                   keepdims=True)
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training:
            self.output = (input - mu)/np.sqrt(sigma + self.EPS)
            self.moving_mean = self.moving_mean * self.alpha + mu * (1 - self.alpha)
            self.moving_variance = self.moving_variance * self.alpha + sigma * (1 - self.alpha)
        else:
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        sum = np.mean(gradOutput * self.output, axis=0)
        self.gradInput = (gradOutput - gradOutput.mean(axis=0) - self.output * sum) / np.sqrt(var + self.EPS)
        return self.gradInput
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        self.output = np.maximum(input, 0) + np.multiply(self.alpha * (np.exp(input) - 1), input < 0)
        return  self.output
    
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.multiply(gradOutput, input > 0) + np.multiply(self.alpha * np.exp(input) * gradOutput, input < 0)
        return self.gradInput
